...
require_once(DUPLICATOR_PRO_PLUGIN_PATH . '/classes/utilities/class.settings.u.php');
	...
	class DUP_PRO_Web_Services
	{
		...
		function duplicator_pro_package_scan()
		{
			...
			else
			{
				...
				DUP_PRO_U::log("Problem opening locking file so auto switching to SQL lock mode");
...
//        }
		...
		function export_settings()
		{
			...
			DUP_PRO_U::log("after referrer check");
			DUP_PRO_Util::CheckPermissions('export');
			...
			try
			{
				...
				$settings_u->run_export();    			
			
				DUP_PRO_U::echo_download_attachment($settings_u->export_filepath, 'application/json');								
			}
			catch (Exception $ex)
			{			
				...
				header("Content-Type: text/plain");
				header("Content-Disposition: attachment; filename=\"error.txt\";");
				$message = DUP_PRO_U::__("Error when running export. {$ex->getMessage()}");
				DUP_PRO_U::log($message);
				echo $message;
			}
			exit;
		...
		function get_package_file()
		{
			...
			check_ajax_referer('duplicator_pro_download_package_file', 'nonce');
