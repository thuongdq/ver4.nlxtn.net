...
require_once(DUPLICATOR_PRO_PLUGIN_PATH . '/classes/utilities/class.db.php');
require_once(DUPLICATOR_PRO_PLUGIN_PATH . '/classes/utilities/class.license.u.php');
require_once(DUPLICATOR_PRO_PLUGIN_PATH . '/classes/utilities/class.alert.u.php');
...
if (is_admin() == true)
{
    ...
    require_once 'classes/ui/class.ui.php';
	...
	function duplicator_pro_admin_footer() 
	{
		...
		$txt_trace_load = DUP_PRO_U::__("Download Log") . ' (' . DUP_PRO_U::get_trace_log_status() . ')';
        $txt_trace_zero = DUP_PRO_U::__("Download Log") . ' (0B)';
        $txt_clear_trace = DUP_PRO_U::__('Clear Log');
		$url = wp_nonce_url('admin.php?page=duplicator-pro-settings&_trace_log_enabled=0&action=save', 'duppro-settings-general-edit', '_wpnonce');
		$html = <<<HTML
			<style>p#footer-upgrade {display:none}</style>
			<div id='dpro-monitor-trace-area'>
				<a href='{$url}'>{$txt_trace_on}</a> <br/>
				<a class='button button-small' href='admin-ajax.php?action=duplicator_pro_get_trace_log'><i class="fa fa-download"></i> <span id='dup_pro_trace_txt'>{$txt_trace_load}</span></a>
...
HTML;
    ...
    if(isset($_REQUEST['page']) && DUP_PRO_U::contains($_REQUEST['page'], 'duplicator-pro'))
    {
    ...
    if(DUP_PRO_U::is_multisite())
	{
		...
		add_action('network_admin_notices', array('DUP_PRO_ALERT_U', 'license_alert_check'));
	}
	else
	{
		add_action('admin_notices', array('DUP_PRO_UI', 'ShowReservedFilesNotice'));
		add_action('admin_notices', array('DUP_PRO_ALERT_U', 'license_alert_check'));
	}
