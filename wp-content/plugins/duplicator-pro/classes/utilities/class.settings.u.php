...
class DUP_PRO_Settings_U
{
	...
	public function run_export()
	{
		...
		{
			DUP_PRO_U::log_object('Error encoding json data for export', $export_data);
	 ...
	 *  @return void */	
	public function run_import($filename, &$opts)
	{
		...
		$this->process_import_data($import_data, $opts);
	...
	private function process_import_data(&$import_data, &$opts)
	{
		$storage_map = null;
		$template_map = null;
				
		DUP_PRO_U::log_object('####opts', $opts);
		foreach($opts as $import_type)
		{
			switch($import_type)
			{
				case 'schedules':					
					if($storage_map === null)
					{
						$storage_map = $this->import_storages($import_data);
					}
					
					if($template_map === null)
					{
						$template_map = $this->import_templates($import_data);
					}
					
					$this->import_schedules($import_data, $storage_map, $template_map);
					break;
	...
	private function import_settings(&$import_data)
	{
		$global = DUP_PRO_Global_Entity::get_instance();
	...
	private function import_templates(&$import_data)
	{
		$template_map = array();
	...
	private function import_schedules(&$import_data, &$storage_map, &$template_map)
	{
	...
	private function import_storages(&$import_data)
	{
		$storage_map = array();
		...
		DUP_PRO_U::log_object('#### Adding storages for import data ', $import_data);
