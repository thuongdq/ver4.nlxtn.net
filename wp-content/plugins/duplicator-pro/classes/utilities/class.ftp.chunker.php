...
class DUP_PRO_FTP_Chunker
{
    ...
    {
		DUP_PRO_U::log("FTP CHUNK OFFSET IN=$offset");
        /* @var $ftp_upload_info DUP_PRO_FTP_UploadInfo */
        ...
        {            
			...
			//		$ftp_size = 1;
                ...
                {
                    DUP_PRO_U::log("timed out so saving off offset $next_offset");
					...
					{
						DUP_PRO_U::log("FTP CHUNK OFFSET OUT=$next_offset");
						$ftp_upload_info->next_offset = $next_offset;
					}
	...
	private function finish_file_chunk($local_file_handle, $next_offset)
	{
		...
		{					
			...
			DUP_PRO_U::log("Finish file chunk next_offset=$next_offset prev_offset=$prev_offset");
		...
		{
			DUP_PRO_U::log("Never was able to finish file chunk transfer");
			return -1;
		}
	}
