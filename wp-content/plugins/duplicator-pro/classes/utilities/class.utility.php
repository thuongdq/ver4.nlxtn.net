...
if (!class_exists('DUP_PRO_Problem_Fix'))
{
...
if (!class_exists('DUP_PRO_Util'))
{
	class DUP_PRO_Util
	{
		...
		// returns array of ['reason'] => ['resolution']
		public static function get_shell_exec_zip_problems()
		{
			$problem_fixes = array();

			if (!self::get_zip_filepath())
			{
				$filepath = null;
...
//					$problem_fix = new DUP_PRO_Problem_Fix();
//
//					$problem_fix->problem = DUP_PRO_U::__('Problem executing shell_exec.');
//					$problem_fix->fix = DUP_PRO_U::__('Ask your host to see why PHP is not able to execute the shell_exec command.');
//
//					$problem_fixes[] = $problem_fix;
//				}
			}
