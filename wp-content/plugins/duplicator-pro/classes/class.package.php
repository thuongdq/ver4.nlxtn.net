...
require_once (DUPLICATOR_PRO_PLUGIN_PATH . 'classes/class.crypt.php');
...
if (!class_exists('DUP_PRO_Package'))
{
	...
	class DUP_PRO_Package
	{
		...
		public function add_log_to_zip($zip_filepath)
		{
			...
			{
				DUP_PRO_U::log("$log_filepath doesnt exist to add to $zip_filepath");
				return true;
			}
		}
...
//				foreach ($this->upload_infos as $upload_info)
		...
		public static function delete_default_local_files($name_hash, $delete_temp, $delete_log_files = true)
		{
			...
			$archive_cfg_temppath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH_TMP . "/{$name_hash}_archive.cfg");
			$archive_zip_temppath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH_TMP . "/{$name_hash}_archive.zip");
			$database_sql_temppath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH_TMP . "/{$name_hash}_database.sql");
			$installer_temppath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH_TMP . "/{$name_hash}_{$global->installer_base_name}");
			$scan_json_temppath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH_TMP . "/{$name_hash}_scan.json");
						
			$archive_cfg_filepath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH . "/{$name_hash}_archive.cfg");
			$archive_zip_filepath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH . "/{$name_hash}_archive.zip");
			$database_sql_filepath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH . "/{$name_hash}_database.sql");
			$installer_filepath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH . "/{$name_hash}_{$global->installer_base_name}");						
			$log_filepath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH . "/{$name_hash}.log");
			$scan_json_filepath = DUP_PRO_Util::SafePath(DUPLICATOR_PRO_SSDIR_PATH . "/{$name_hash}_scan.json");
		...
		public static function get_from_json($json_string)
		{
			...
			DUP_PRO_U::simple_object_copy($stdobject->Archive->FilterInfo->Dirs, $package->Archive->FilterInfo->Dirs);
			DUP_PRO_U::simple_object_copy($stdobject->Archive->FilterInfo->Exts, $package->Archive->FilterInfo->Exts);
			DUP_PRO_U::simple_object_copy($stdobject->Archive->FilterInfo->Files, $package->Archive->FilterInfo->Files);
		...
		public function save()
		{
			...
			if ($this->ID == -1 || empty($this->ID))
			{
				...
				if ($results === false)
				{
					DUP_PRO_U::log("Problem inserting package: {$wpdb->last_error}");
		...
		public function build()
		{
			...
			else if (!$this->build_progress->archive_built) //&& ($this->build_progress->timed_out($global->php_max_worker_time_in_sec) == false))
			{
				$this->Archive->build($this, $this->build_progress);
								
				$this->update();
			}
			else if (!$this->build_progress->installer_built)// && ($this->build_progress->timed_out($global->php_max_worker_time_in_sec) == false))
			{
		...
		public function build_integrity_check()
		{
			...
			if ($this->Archive->file_count != -1)
			{
				...
				if ($expected_filecount > 500)
				{
					...
					DUP_PRO_U::log("Warn/unread counts) warnfile:{$scanReport->ARC->WarnFileCount} warndir:{$scanReport->ARC->WarnDirCount} unreadfile:{$scanReport->ARC->UnreadableFileCount} unreaddir:{$scanReport->ARC->UnreadableDirCount}");
			...
			{
				DUP_PRO_U::log("Running ZipArchive consistency check");
		...
		public static function set_temporary_package_from_template_and_storages($template_id, $storage_ids, $name)
		{
			...
			{
				$package = new DUP_PRO_Package();
				$dbversion = DUP_PRO_DB::mysqlVersion();
				$dbversion = is_null($dbversion) ? '- unknown -' : $dbversion;
                $dbcomments  = DUP_PRO_DB::mysqlVariable('version_comment');
			...
			{
				DUP_PRO_U::log('Template ' . $manual_template->id . "doesn't exist!");
			}
		}
