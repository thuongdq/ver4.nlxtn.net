...
class DUP_PRO_Server 
{
	...
	*/
	public static function get_checks($package) 
    {
		...
		$license		= DUP_Pro_License_Utility::get_license_type();
	...
	*/
	public static function install_files_found() 
	{
		...
		DUP_PRO_IO::delete_file(DUPLICATOR_PRO_WPROOTPATH .  DUPLICATOR_PRO_EMBEDDED_SCAN_FILENAME);
	...
	*/
	public static function get_orphaned_package_files() 
	{
		...
		if(defined('GLOB_BRACE'))
		{
			...
			if($filepaths !== false)
			{
			//	DUP_PRO_U::log_object("orphaned filepaths", $filepaths);
				...
				foreach($filepaths as $filepath) 
				{
					...
					{				
						...
						if(DUP_PRO_U::starts_with($filename, $package->NameHash) && $default_storage_present)
						{
							$is_orphan = false;
							break;
						}
						else if(DUP_PRO_U::starts_with($filename, $package->NameHash) && DUP_PRO_U::ends_with($filename, '.log'))
						{
			...
			{
				DUP_PRO_U::log("orphaned filepaths == false");
			}
		}
		else
		{
			DUP_PRO_U::log("Glob brace not defined");
		}
		return $orphans;
	}
